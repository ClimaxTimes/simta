...
const update = asyncHandler(async (req, res) => {
    ...
    if (isAdmin) {
        ...
        allowedFields = ['name', 'email', 'prodi', 'semester', 'judulTA', 'currentProgress', 'status', 'avatar'];
    } else {
...
/**
 * @desc    Get mahasiswa bimbingan (for dosen)
...
const getAllDosen = asyncHandler(async (req, res) => {
    const dosen = await User.find({ role: 'dosen', status: 'aktif' })
        .select('name nim_nip email')
        .sort({ name: 1 });
